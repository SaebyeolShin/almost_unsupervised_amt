...
from utils import (
    load_vae_from_checkpoint, list_split_files,
    piano_metrics_from_rolls, ImagePool,
    UnalignedCqtRollDataset, PairedCqtRollDataset,
)



class CqtRollCycleLightning(pl.LightningModule):
    def __init__(self, cfg, image_dir):
        super().__init__()
        ...
        self.z_channels = self.vae.z_channels
    ...
    def _encode_midi_latent(self, midi_roll: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            mean, logvar = self.vae.encode(midi_roll)
    ...
    def _compute_metrics(self, fake_midi_z, real_midi_roll):
        with torch.no_grad():
            fake_midi_roll = self.vae.decode(fake_midi_z)

        fake_cpu = fake_midi_roll.detach().float().cpu().numpy()
        real_cpu = real_midi_roll.detach().float().cpu().numpy()

        frame_f1s = [
            piano_metrics_from_rolls(
                (real_cpu[i, 0] > 0).astype(int),
                (fake_cpu[i, 0] > 0).astype(int),
            )["Frame F1"]
            for i in range(len(fake_cpu))
        ]
        return {"frame_f1": np.mean(frame_f1s)}
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            mean, logvar = self.vae.encode(real_midi_roll)
            real_midi_latent = self.vae.reparameterize(mean, logvar).detach()
        ...
        # 2. EMA Model (if enabled)
        if self.use_ema:
             fake_midi_z_ema, rec_cqt_ema, _, rec_midi_z_ema = self.forward(real_cqt, real_midi_latent, use_gen_ema=True)
    ...
    def _save_cycle_images(self, real_cqt, fake_midi, rec_cqt, real_midi_roll, fake_cqt, rec_midi, max_samples=4, tag="train"):
        ...
        with torch.no_grad():
            fake_midi_roll = self.vae.decode(fake_midi[:bsz])
            rec_midi_roll  = self.vae.decode(rec_midi[:bsz])
        ...
        real_cqt_np = _to_np(real_cqt)
        fake_midi_np = _to_np(fake_midi_roll) # Decoded
        rec_cqt_np  = _to_np(rec_cqt)
        real_midi_np = _to_np(real_midi_roll) # Original roll
        fake_cqt_np = _to_np(fake_cqt)
        rec_midi_np  = _to_np(rec_midi_roll) # Decoded
        ...
        im = Image.fromarray(img_255, mode="L")

        step = int(self.global_step)
        out_path = self.image_dir / f"{tag}_epoch{self.current_epoch:03d}_step{step:07d}.png"
        im.save(out_path)
        print(f"[viz] saved inverted image grid to {out_path}")

# ── main / CLI ──────────────────────────────────────────────────────────────

def make_fixed_paired_subset(train_cqt, train_midi, paired_fraction: float, seed: int):
    assert len(train_cqt) == len(train_midi)
    n = len(train_cqt)
    k = max(1, int(round(n * paired_fraction)))
    rng = np.random.RandomState(seed)
    idx = rng.choice(n, size=k, replace=False)
    idx = np.sort(idx)
    paired_cqt = [train_cqt[i] for i in idx]
    paired_midi = [train_midi[i] for i in idx]
    return paired_cqt, paired_midi, idx

@hydra.main(version_base=None, config_path="configs", config_name="cycle_piano")
def main(cfg: DictConfig):
    ...
    train_cqt, train_midi = list_split_files(data_root, split="train")
    val_cqt,   val_midi   = list_split_files(data_root, split="validation")
    test_cqt,  test_midi  = list_split_files(data_root, split="test")

    print(f"[main] train pairs={len(train_cqt)}, val pairs={len(val_cqt)}, test pairs={len(test_cqt)}")

    val_ds   = PairedCqtRollDataset(val_cqt, val_midi)
    test_ds  = PairedCqtRollDataset(test_cqt, test_midi)

    paired_frac = float(cfg.training.get("paired_fraction", 0.01))
    paired_cqt, paired_midi, paired_idx = make_fixed_paired_subset(
        train_cqt, train_midi, paired_fraction=paired_frac, seed=cfg.seed
    )

    np.save(os.path.join(ckpt_dir, "paired_idx.npy"), paired_idx)
    
    # Unpaired CQT: optionally mix in a second dataset (e.g. GuitarSet).
    # Set data.guitar_root in the config to enable mixing.
    guitar_root = cfg.data.get("guitar_root", None)
    if guitar_root:
        guitar_train_cqt = sorted((Path(guitar_root) / "train" / "audio").glob("*.npy"))
        unpaired_cqt = train_cqt + guitar_train_cqt
        unpaired_midi = train_midi
        print(f"[main] GuitarSet mixing enabled: +{len(guitar_train_cqt)} guitar CQT files")
    else:
        # MAESTRO-only: exclude paired indices from the unpaired pool
        unpaired_mask = np.ones(len(train_cqt), dtype=bool)
        unpaired_mask[paired_idx] = False
        unpaired_cqt  = [train_cqt[i]  for i in range(len(train_cqt))  if unpaired_mask[i]]
        unpaired_midi = [train_midi[i] for i in range(len(train_midi)) if unpaired_mask[i]]
        print(f"[main] MAESTRO-only: {len(unpaired_cqt)} / {len(train_cqt)} unpaired (paired indices excluded)")

    train_unpaired_ds = UnalignedCqtRollDataset(unpaired_cqt, unpaired_midi)
    train_paired_ds   = PairedCqtRollDataset(paired_cqt, paired_midi)
    ...
    if resume_ckpt:
        ...
        if cfg_path.exists():
            print(f"[resume] loading config from {cfg_path}")
            old_cfg = OmegaConf.load(cfg_path)

            for k in [
                "training.resume_ckpt",
                "training.extra_steps",
                "run_name",
                "logging",
            ]:
            ...
            print("[resume] WARNING: no config.yaml next to ckpt, using current cfg")

    model = CqtRollCycleLightning(cfg, image_dir=os.path.join(out_dir, "images"))
    ...
    best_ckpt_cb = ModelCheckpoint(
        ...
        filename="best-loss",
        save_top_k=1,
        monitor="val/val_loss_ema",
        mode="min",
    )
    best_f1_cb = ModelCheckpoint(
        dirpath=ckpt_dir,
        filename="best-f1",
        save_top_k=1,
        monitor="val/frame_f1_ema",
        mode="max",
    )
    ...
    extra_steps = int(cfg.training.get("extra_steps", 0))
    trainer = pl.Trainer(
        max_steps=int(cfg.training.num_train_steps) + extra_steps,
        ...
        callbacks=[latest_ckpt_cb, best_ckpt_cb, best_f1_cb, lr_cb, pbar],
        logger=logger,
    ...
    trainer.fit(model, train_loader, val_loader, ckpt_path=str(resume_ckpt) if resume_ckpt else None)
